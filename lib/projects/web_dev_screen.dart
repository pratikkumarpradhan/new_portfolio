...
class _PortfolioWebScreenState extends State<PortfolioWebScreen> {
  ...
  void _listenToAuthState() {
    _authSubscription = FirebaseAuth.instance.authStateChanges().listen((user) {
      debugPrint('🔍 PortfolioWebScreen auth state changed: ${user?.email}');
      _checkAdmin(user);
    }, onError: (error) {
      debugPrint('❌ PortfolioWebScreen auth state error: $error');
      setState(() {
        _isAdmin = false;
        _loading = false;
      });
  ...
  Future<void> _checkAdmin(User? user) async {
    ...
    try {
      ...
      debugPrint('❌ Error checking admin in PortfolioWebScreen: $e');
      setState(() {
        _isAdmin = false;
        _loading = false;
      });
