...
//   @override
//   State<LeetCodeContributionsWidget> createState() =>
//       _LeetCodeContributionsWidgetState();
// }
...
//     const url = 'https://api.LeetCode.com/graphql';
...
//       return yearDays;
//     } else {
//       debugPrint('LeetCode API error: ${response.body}');
//       if (mounted) {
//         setState(() {
...
class LeetCodeContributionsWidget extends StatefulWidget {
  ...
  const LeetCodeContributionsWidget({
    ...
    required this.username,
    this.height = 200,
    this.contributionColors = const [
      Color(0xFF9BE9A8),
      Color(0xFF40C463),
      Color(0xFF30A14E),
      Color(0xFF216E39),
    ],
    this.emptyColor = const Color(0xFF161B22),
...
class _LeetCodeContributionsWidgetState
    extends State<LeetCodeContributionsWidget> {
  bool _isLoading = true;
  String? _error;
  ...
  void initState() {
    super.initState();
    _fetchLeetCodeContributions();
  }

 Future<void> _fetchLeetCodeContributions() async {
  setState(() {
    _isLoading = true;
    _error = null;
    _contributions = [];
  ...
  try {
    final url = Uri.parse(
        'https://leetcode-api-autd.onrender.com/leetcode/${widget.username}'); // Node API
    ...
    List<ContributionDay> parsed = [];

    decoded.forEach((timestamp, count) {
      final dt = DateTime.fromMillisecondsSinceEpoch(int.parse(timestamp) * 1000);
      parsed.add(ContributionDay(date: dt, count: count as int));
    });
  ...
  Color _getColor(int count) {
    if (count == 0) return widget.emptyColor;
    final index = (count / 5).clamp(0, widget.contributionColors.length - 1).toInt();
    return widget.contributionColors[index];
  }
  ...
  Widget _buildGrid() {
    // Group by week
    Map<int, List<ContributionDay>> weeks = {};
    for (var c in _contributions) {
      int week = _weekOfYear(c.date);
      weeks.putIfAbsent(week, () => []).add(c);
    }
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: entry.value.map((c) {
                return Tooltip(
                  message:
                      '${DateFormat('MMM d, yyyy').format(c.date)}: ${c.count} submissions',
