...
class _LearningScreenState extends State<LearningScreen> {
  ...
  void _listenToAuthState() {
    _authSubscription = FirebaseAuth.instance.authStateChanges().listen((user) {
      debugPrint('🔍 ExperienceScreen auth state changed: ${user?.email}');
      _checkAdmin(user);
    }, onError: (error) {
      debugPrint('❌ ExperienceScreen auth state error: $error');
  ...
  Future<void> _checkAdmin(User? user) async {
    ...
    try {
      ...
      debugPrint('❌ Error checking admin in ExperienceScreen: $e');
      setState(() => _isAdmin = false);
    }
  }
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final docs =
                  await FirebaseFirestore.instance.collection('journeySteps').get();
